...
class _PMDB{
      ...
      public:
             ...
             void Set_DB(char db){
                        ...
                        if (DB_config.size() == 0){
                                             Rcpp::Rcout << "Error: Cannot open reference database configuration" << endl;
                                             return;
                                             }
                        ...
                        //debug
                        //Rcpp::Rcout << a_config.size() << endl;
                        //for (int i = 0; i < a_config.size(); i ++)
                        //    Rcpp::Rcout << a_config[i] << endl;
...
unsigned int _PMDB::Read_Taxonomy(hash_map<string, string, std_string_hash> & table){
                       ...
                       if (!infile){
                          Rcpp::Rcout << "Error: Open Taxonomy Annotation file error : " << DB_taxa_anno << endl;
                          Rcpp::stop("");
                       }
                       ...
                       while (getline(infile, buffer)){
                             ...
                             */
                             ...
                             if (table.count(id) > 0) {
                                                 ...
                                                 Rcpp::Rcout << "Error: Loading Taxonomy Annotation error : Duplicate ID : " << id << endl;
                              			 Rcpp::stop(""); 
                                                 }
                               else (table)[id] = taxa;
...
int _PMDB::Load_Copy_Number(hash_map <string, float, std_string_hash> & cp_number){
                ...
                if (!in_cp_number){
                       Rcpp::Rcout << "Warning: Cannot open copy number table : " << DB_cp_number << ", copy number normalization is disabled" << endl;
                       return 0;
                       } 
                ...
                while(getline(in_cp_number, buffer)){
                                ...
                                if (cp_number.count(id) == 0)
                                                           cp_number[id] = cp_no;
                                else Rcpp::Rcout << "Warning, dup id : " << id << endl;
...
int _PMDB::Load_config(){
       ...
       //debug
       //Rcpp::Rcout << "Loading db config" << endl;
       ...
       //debug
       //Rcpp::Rcout << "Loading db config complete: " << DB_config.size() << endl;
