...
p <- c("optparse","hrms", "ggplot2", "grid")
...
PM_pcoa <- function(da, md) {
  ...
  colnames(pcoa$pc)<-c("PC1","PC2","PC3")
  rownames(pcoa$pc)<-rownames(dst_orig)
  data<-as.data.frame(pcoa$pc)
  loadings <- signif(pcoa$per, digits=3)
